...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function is LossFunction.MSE:
            ...
            arg = x @ self.w - y
            res = np.mean(abs(arg) + np.log1p(np.exp(-2 * abs(arg))) - np.log(2))
#             print(res)
            return res
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        if self.loss_function is LossFunction.MSE:
            return 2 * (x.T @ x @ self.w - x.T @ y) / x.shape[0]
        elif self.loss_function is LossFunction.LogCosh:
#             a = np.dot(x, self.w)
#             exp_term = np.exp(2 * (a - y))
#             gradient = np.dot(x.T, (exp_term - 1) / (exp_term + 1))
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function is LossFunction.MSE:
            return 2 * (x[indices].T @ x[indices] @ self.w - x[indices].T @ y[indices]) / self.batch_size
        elif self.loss_function is LossFunction.LogCosh:
            return x[indices].T @ np.tanh(x[indices] @ self.w - y[indices]) / self.batch_size
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        return (-1) * self.lr() * m_hat / (v_hat**0.5 + self.eps)
